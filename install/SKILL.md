...
One operator gate: **the bar**. Everything after it runs unattended. A wrong bar wastes the whole run: every
builder decision, every critic verdict and every stop condition derives from it. Nothing else is worth
...
threshold, a run path, a ceiling or a modality capability — each is referenced by contract number, and the
operator-facing text names the file that holds the value instead of copying it. Where this file and a
contract disagree, this file is the bug.

## Reference map — load on demand, never inline

An agent that has read 3,000 lines of prescription writes worse prompts than one that read the goal. Load a
row at the step that needs it, not before.
...
Siblings resolve as `SKILL_DIR/<FILE>.md` (C3.1) — no `../` segment anywhere. A file is missing: do not
improvise depth, run the Invariants below and name the absent file.
...
3. **Select the modality.** C5 decides it; `LAUNCH.md` §1 STEP 2b is its router, `INSPECTION.md` has the
   ...
   is missing. Never improvise a phrase, never order an inspection C5 does not grant, and never soften an
   excluded capability into a paraphrase that sounds performable: the deaf critic — a prompt telling a critic
   to judge a recording — is the defect this rule exists for, and audio, motion with no time hook, perceived
   latency, sustained load and read-aloud each take the fate C5.3 assigns them. A still image cannot find a
   behavioural bug: if the goal is behavioural, the row is behavioural, whatever the artifact looks like.
...
6. **Open the run, then launch.** On approval, in this order:
   1. Create the run directory and **the C3.2 tree, complete**, `current` symlink included; run-id in the
      dated form `ROLES.md`'s slots show. Enumerate that tree nowhere else: any path the round-close audit
      greps that a second list omits makes the check match nothing and report a clean round, which is the
   ...
   2. **Freeze the bar.** Capture the named artifact into `bar/` **once** — screenshot, recording, download,
      ...
      Layout per `BARS.md`, which also requires `PROVENANCE.md` when the bar is synthetic. Hash it with
      **C3.4's cwd-pinned command**; a relative `shasum` reports drift on an unchanged bar. Critics read that
      snapshot, never the live source; a re-fetch mid-run is a new run (F3), and without `bar.sha256` no
      result can be shown to have reproduced against the same bar.
   ...
   4. Fresh-spawn the `ROLES.md` decomposer for round zero only, on the tier `ROLES.md` § Spawn rules
      assigns: decompose and register the parts, no loop. `{{GOAL}}` = the approved goal line; `{{BAR}}` =
      the bar name as recorded in `run.json`; `{{BUDGET}}` from the gate, in the output-token unit C4 fixes;
      `launch-prompt.md` attached verbatim as the charter. The decomposer — not you — fills `{{PART}}`,
      `{{DIMENSION}}`, `{{PROBE}}`, `{{TARGET_USER}}`, `{{A_PATH}}`/`{{B_PATH}}`, `{{WORKTREE}}`.
   5. Hand the registered parts to a **Workflow** script and run that. Never hand-schedule rounds from this
      ...
      `runId` and `scriptPath` in `run.json` so `--resume` has a handle, and append the round's `spend.tsv`
      row at close with C3.2's columns **in C3.2's units** — the wrong unit in the right number of columns
      leaves F21's breach check comparing two different currencies across rounds (C4.4).
...
Probes:    <probe 1>, <probe 2>   C5 rows: <row>, <row>   evidence: <what the critic looks at>
           [HUMAN GATE: <file> — <what a person must hear or see>, whenever the row leaves a half
            no tool on this machine reaches]
...
Copy the whole system, not just this file: every sibling must land directly beside `SKILL.md` or the
reference map resolves to nothing (C3.1); what survives installation, and why the baseline lands under
`bars/`, is C3.8. Project-scoped copy: same layout under `<repo>/.claude/skills/gauntlet/`, which commits the
bar library with the code and overrides the personal copy in that repo.
...
| `ls` shows `gauntlet/gauntlet-loop-system/SKILL.md` | `cp -R` ran without the trailing `/.`, so the repo landed as a nested directory. `rm -rf ~/.claude/skills/gauntlet` and re-run the block |
...
- When the bar is `bars/baseline-gauntlet-prompt.md`, C5.6 governs what may be claimed about the result: no
  run and no report may say that baseline has been beaten until the blind A/B of emitted launch prompts has
  actually run.
